...
import datasets
import numpy as np
from peft import LoraConfig, TaskType, get_peft_model, prepare_model_for_kbit_training, PeftModel,prepare_model_for_int8_training
...
def build_model(model_args, training_args, checkpoint_dir):
    ...
    model = transformers.AutoModelForCausalLM.from_pretrained(
        ...
        # load_in_4bit=model_args.bits == 4, 
        # load_in_8bit=model_args.bits == 8,
        # quantization_config=bnb_config if model_args.bits==4 else None, 
        torch_dtype=compute_dtype,
        trust_remote_code=True,
        # cache_dir = "/mnt/nlp/gaoqiang/ckpt/deepseek"
    )
    ...
    if model_args.use_lora and model_args.bits==8:
        model = prepare_model_for_int8_training(model)
    # elif model_args.load_in_bits==4:
    #     model = prepare_model_for_kbit_training(model)
    elif model_args.use_lora and model_args.bits < 8:
...
def train():
    ...
    if training_args.local_rank == 0:
        ...
        for index in random.sample(range(len(train_dataset)), 3):
            ...
            # logger.info("Eval dataset samples:", len(eval_dataset))
        # for index in random.sample(range(len(eval_dataset)), 3):
        #     logger.info(f"Sample {index} of the eval set: {eval_dataset[index]['input_ids']}, {train_dataset[index]['labels']}.")
        #     logger.info(f"Sample {index} of the eval set: {tokenizer.decode(list(train_dataset[index]['input_ids']))}.")
